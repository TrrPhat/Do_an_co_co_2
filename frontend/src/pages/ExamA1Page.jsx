...
export default function ExamA1Page() {
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    const url = new URL(`${API_BASE}/api/v1/exams/hang-a1/generate`);
    ...
    fetch(url.toString())
      ...
      .then(data => {
        setExam(data);
        setRemaining(data?.meta?.time_limit ?? 19*60);
        setCurrent(1);
        setAnswers({});
        setSubmitted(false);
        setResult(null);
  ...
  function doSubmit() {
    ...
    const pass = score >= 21 && !wrongLiability; // A1: 21/25, không sai câu liệt
    ...
    <div className="min-h-screen bg-white font-body text-slate-900">
      <div className="max-w-7xl mx-auto px-4 py-24">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="font-display text-2xl">Thi thử hạng A1 - 25 câu</h1>
            <div className="text-slate-600 text-sm">Thời gian 19 phút • Đạt 21/25 • Sai 1 câu điểm liệt là KHÔNG ĐẠT</div>
          </div>
        ...
        {submitted && result && (
          <div className={`mb-4 p-4 rounded-lg border ${result.pass ? 'bg-green-50 border-green-500' : 'bg-rose-50 border-rose-500'}`}>
            ...
            <div className="mt-2 flex gap-2">
              ...
              <Link to="/lythuyet/hang-a1/de/random" className="px-3 py-1.5 rounded-md bg-slate-900 text-white">Đề ngẫu nhiên khác</Link>
            </div>
          </div>
        )}
            ...
            <div className="lg:col-span-8 xl:col-span-9">
              <div className="bg-white rounded-2xl border border-slate-200 shadow">
                <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200">
                  <div className="font-heading text-slate-950 text-lg">Câu hỏi {exam.questions[current-1]?.id}</div>
